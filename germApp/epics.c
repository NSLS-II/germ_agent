...
void * epics_thread(void * arg)
{
    ...
    int status;
    enum thread_state_t {IDLE, STAGE, WAIT_FOR_SCAN, SCAN} thread_state = IDLE;
    ...
    memcpy(pv_suffix[PV_FILENAME],     "FNAM",                4);
    memcpy(pv_suffix[PV_FILESIZE],     "Data:Filesize",      13);
    memcpy(pv_suffix[PV_RUNNO],        "RUNNO",               5);
    ...
    memcpy(pv_suffix[PV_FILENAME_ACK], "FNAM_ACK",            8);
    memcpy(pv_suffix[PV_FILESIZE_ACK], "FSIZ_ACK",            8);
    memcpy(pv_suffix[PV_RUNNO_ACK],    "RUNNO_ACK",           9); 
